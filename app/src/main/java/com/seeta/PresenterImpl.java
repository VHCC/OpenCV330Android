...
import com.seeta.config.CachedStatusAndImage;
...
import static acl.siot.opencvwpc20191007noc.VMSMainActivity.MODELPATH;
import static com.seeta.config.AppConfig.CAMERA_PREVIEW_HEIGHT;
import static com.seeta.config.AppConfig.CAMERA_PREVIEW_WIDTH;
...
//    FaceAntiSpoofing.Status state = FaceAntiSpoofing.Status.DETECTING;//初始状态
    ...
    static String mdModel = MODELPATH + "/mask_detector.csta";
...
//				faceAntiSpoofing.SetThreshold(clarity, fasThresh);
            ...
            mLog.d(TAG, "PROPERTY_MIN_FACE_SIZE:> " + faceDetector.get(FaceDetector.Property.PROPERTY_MIN_FACE_SIZE));
            mLog.d(TAG, "PROPERTY_THRESHOLD:> " + faceDetector.get(FaceDetector.Property.PROPERTY_THRESHOLD));
//            if (maskDetector == null) {
//                try {
//                    maskDetector = new MaskDetector(new SeetaModelSetting(0, new String[]{mdModel}, SeetaDevice.SEETA_DEVICE_AUTO));
//                } catch (Exception e) {
...
//            trackingInfo.matBgr.create(new Size(WIDTH, HEIGHT), CvType.CV_8UC4);
            ...
            mLog.d("seetatech", "faces:> " + faces.length);
            ...
            if (faces.length != 0 ) {
                ...
                mLog.d("seetatech", "faceRect.x:> " + faces[maxIndex].x);
                trackingInfo.faceRect.y = faces[maxIndex].y;
                mLog.d("seetatech", "faceRect.y:> " + faces[maxIndex].y);

                trackingInfo.faceRect.width = faces[maxIndex].width;
                mLog.d("seetatech", "faceRect.width:> " + faces[maxIndex].width);
